...
fn try_enumeration(input: TokenStream) -> Result<TokenStream, Error> {
    ...
    let display = variants.iter().zip(proto_names.iter()).map(
        |(&(ref variant, _), proto_name)| quote!(#ident::#variant => f.write_str(#proto_name)),
    );
    ...
    let expanded = quote! {
        ...
        impl #impl_generics ::prost::alloc::fmt::Display for #ident #ty_generics #where_clause {
            fn fmt(&self, f: &mut ::prost::alloc::fmt::Formatter) -> ::prost::alloc::fmt::Result {
                match self {
                    #(#display,)*
                }
            }
